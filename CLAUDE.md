...
- 若教訓是**通則**，再把它**蒸餾進對應章節**（例：分支基底→§2、跨區掛 quest→§4）。`lessons/` 留完整事件、章節留精煉守則。
- **動手前掃一下 `lessons/`**，尤其在「查證內容／改資料／部署」這幾類重複性高、踩過雷的事情上。
- 能變成**自動檢查／腳本**的就別只靠文件（治本）；但至少先記下來（治標）。
...
- **分支制協作（2026-06 更新，取代先前「直接在 `main` 上 commit、不用 feature 分支」）：**
  - **每個 agent 的改動都 commit 在自己的工作分支**（如 `claude/<task>-xxxx`），**不直接 commit `main`**。
  - **`main` 是大家共用的 base branch**；**只要 `main` 有新進度，就把當前分支 rebase 到最新
    `origin/main`**（呼應上面🔴鐵則：落後就先 rebase 再繼續，絕不蓋在舊基底上）。
  - 仍是單人專案：無獨立 staging、不跑本地測試（驗證四件套由 Claude 把關，見 §3）、出問題用 git 切回去。
  - 推分支或推 `main` 本身都**不**觸發部署；**要發布到線上＝把分支併進 `main` 後打 tag**（見下兩條）。
  - ※ 為何不走「直接 commit `main`」的捷徑：**AI agent 的 system prompt 本來就不允許**——
    任務設定會指定工作分支、且明令「未經允許不得推到其他分支」，在預設分支上也要求先開分支再動。
    所以分支制是**被迫、也是正解**（同時也是多 session 平行作業不互相覆蓋的唯一做法）。
...
### 🧭 新增／跨區內容的放置原則（2026-06，Adula 事故後立規）
線性流程是**忠實照攻略作者的「路線順序」**走（那順序本來就會在區域間交錯），**「章節＝地區」只是段落標籤，不是「同區一次做完」的承諾**。所以：
- **新增步驟一律插在它在路線上「真正發生」的時點**，**不要為了好放就按地區 append**（曾把輝石龍亞杜拉的首次遭遇＋擊殺都硬塞同一章，導致「擊殺在前、遭遇在後」的怪序）。
- **跨區的鏈**（A 區遇到 → 跑別區 → 回來解決）要做兩件事：①每步放各自的真實時點；②把這些步驟 id 加進對應 `quests[].stepIds`（並給 step `quests:[id]`），讓「支線總覽」聚合成跨區時間軸。`stepIds` 要**依流程順序**排，並同步更新該 quest 的 `count`。
- 範例：Adula 首次遭遇=`ch8`(三姊妹塔)、擊殺=`ch16`(月光祭壇)，兩步都掛 `ranni`。
